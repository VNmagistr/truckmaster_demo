...
class ServiceReminderAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Інтервал повторення', {
            ...
            'description': (
                'Інтервал за пробігом підтягується автоматично з Комплекту ТО авто '
                '(поле "Інтервал заміни оливи"). '
                'Часовий інтервал вкажіть вручну якщо потрібен, або залишіть порожнім — '
                'береться з типу ТО.'
            ),
        }),
        ('Статус', {
