...
export default function OrdersPage() {
    ...
    const fetchData = useCallback(async () => {
        try {
            const [ordersRes, clientsRes, trucksRes] = await Promise.all([
                axiosInstance.get('/service-orders/'),
                axiosInstance.get('/clients/'),
                axiosInstance.get('/trucks/')
            ]);
            
            const ordersData = ordersRes.data;
            if (ordersData && Array.isArray(ordersData.results)) { setOrders(ordersData.results); } 
            else if (Array.isArray(ordersData)) { setOrders(ordersData); }
    ...
    const handleOpenModal = () => {
        setFormData({ status: 'new', description: '', client: '', truck: '' });
        setOpen(true);
    };
    ...
    const handleInputChange = (event) => {
        ...
        if (name === 'client') {
            setFormData(prevState => ({ ...prevState, client: value, truck: '' }));
        } else {
    ...
    return (
        <Container style={{ marginTop: '2rem' }}>
            <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                ...
                <Button variant="contained" color="primary" onClick={handleOpenModal}>Створити замовлення</Button>
            </Box>
            ...
            <Dialog open={open} onClose={handleCloseModal} fullWidth maxWidth="sm">
                ...
                <DialogContent>
                    ...
                    <FormControl fullWidth margin="dense" variant="standard" disabled={!formData.client}><InputLabel>Автомобіль</InputLabel><Select name="truck" value={formData.truck || ''} onChange={handleInputChange}>{filteredTrucks.map(truck => <MenuItem key={truck.id} value={truck.id}>{truck.specific_model_name} ({truck.license_plate})</MenuItem>)}</Select></FormControl>
                    ...
                    <FormControl fullWidth margin="dense" variant="standard"><InputLabel>Статус</InputLabel><Select name="status" value={formData.status || 'new'} onChange={handleInputChange}><MenuItem value="new">Нове</MenuItem><MenuItem value="in_progress">В роботі</MenuItem><MenuItem value="completed">Завершено</MenuItem><MenuItem value="canceled">Скасовано</MenuItem></Select></FormControl>
                </DialogContent>
